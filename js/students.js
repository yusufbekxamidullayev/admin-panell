...
async function getStudents() {
  try {
    ...
    res.data.map(el => {
      studentsContainer.innerHTML += `
                <div class="group relative w-[320px] h-[270px] overflow-hidden rounded-lg shadow-md">
...
async function deleteStudent(id) {
  try {
    select = id;
    await axios.delete(`https://69135d0ef34a2ff1170bb5ac.mockapi.io/Teachers/${teacherId}/students/${select}`);
    getStudents();
  } catch (err) {
    console.log(err);
  }
